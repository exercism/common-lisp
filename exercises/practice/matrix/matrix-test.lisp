...
(test extract-row-from-one-number-matrix
    (let ((input-array #2A((1)))
          (index 1))
      (is (equal (list 1) (matrix:row input-array index)))))

(test can-extract-row
    (let ((input-array #2A((1 2)
                           (3 4)))
          (index 2))
      (is (equal (list 3 4) (matrix:row input-array index)))))

(test extract-row-where-numbers-have-different-widths
    (let ((input-array #2A((1 2)
                           (10 20)))
          (index 2))
      (is (equal (list 10 20) (matrix:row input-array index)))))

(test can-extract-row-from-non-square-matrix-with-no-corresponding-column
    (let ((input-array #2A((1 2 3)
                           (4 5 6)
                           (7 8 9)
                           (8 7 6)))
          (index 4))
      (is (equal (list 8 7 6) (matrix:row input-array index)))))

(test extract-column-from-one-number-matrix
    (let ((input-array #2A((1)))
          (index 1))
      (is (equal (list 1) (matrix:column input-array index)))))
...
(test can-extract-column
    (let ((input-array #2A((1 2 3)
                           ...
                           (7 8 9)))
          (index 3))
      (is (equal (list 3 6 9) (matrix:column input-array index)))))

(test can-extract-column-from-non-square-matrix-with-no-corresponding-row
    (let ((input-array #2A((1 2 3 4)
                           (5 6 7 8)
                           (9 8 7 6)))
          (index 4))
      (is (equal (list 4 8 6) (matrix:column input-array index)))))

(test extract-column-where-numbers-have-different-widths
    (let ((input-array #2A((89 1903 3)
                           (18 3 1)
                           (9 4 800)))
          (index 2))
      (is (equal (list 1903 3 4) (matrix:column input-array index)))))
