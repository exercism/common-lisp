...
(test no-multiples-within-limit
    (let ((factors (list 3 5))
          (limit 1))
      (is (equal 0 (sum-of-multiples:sum factors limit)))))
...
(test more-than-one-multiple-within-limit
    (let ((factors (list 3))
          (limit 7))
      (is (equal 9 (sum-of-multiples:sum factors limit)))))

(test more-than-one-factor-with-multiples-within-limit
    (let ((factors (list 3 5))
          (limit 10))
      (is (equal 23 (sum-of-multiples:sum factors limit)))))

(test each-multiple-is-only-counted-once
    (let ((factors (list 3 5))
          (limit 100))
      (is (equal 2318 (sum-of-multiples:sum factors limit)))))

(test a-much-larger-limit
    (let ((factors (list 3 5))
          (limit 1000))
      (is (equal 233168 (sum-of-multiples:sum factors limit)))))

(test three-factors
    (let ((factors (list 7 13 17))
          (limit 20))
      (is (equal 51 (sum-of-multiples:sum factors limit)))))

(test factors-not-relatively-prime
    (let ((factors (list 4 6))
          (limit 15))
      (is (equal 30 (sum-of-multiples:sum factors limit)))))

(test some-pairs-of-factors-relatively-prime-and-some-not
    (let ((factors (list 5 6 8))
          (limit 150))
      (is (equal 4419 (sum-of-multiples:sum factors limit)))))

(test one-factor-is-a-multiple-of-another
    (let ((factors (list 5 25))
          (limit 51))
      (is (equal 275 (sum-of-multiples:sum factors limit)))))

(test much-larger-factors
    (let ((factors (list 43 47))
          (limit 10000))
      (is (equal 2203160 (sum-of-multiples:sum factors limit)))))

(test all-numbers-are-multiples-of-1
    (let ((factors (list 1))
          (limit 100))
      (is (equal 4950 (sum-of-multiples:sum factors limit)))))
